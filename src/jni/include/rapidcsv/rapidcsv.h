...
namespace rapidcsv
{
	...
	class no_converter : public std::exception
	{
	  public:
		 ...
		 */
		const char* what() const throw() override { return "unsupported conversion datatype"; }
	};
	  ...
	  public:
		...
		void ToStr(const T& pVal, std::string& pStr) const
		{
			if (typeid(T) == typeid(int) || typeid(T) == typeid(long) || typeid(T) == typeid(long long) ||
				...
				typeid(T) == typeid(unsigned long long) || typeid(T) == typeid(long double) ||
		...
		void ToVal(const std::string& pStr, T& pVal) const
		{
			try
			{
				...
				{
					pVal = static_cast<T>(strtoul(pStr.c_str(), NULL, 10));
					return;
				}
				...
				{
					pVal = static_cast<T>(strtoull(pStr.c_str(), NULL, 10));
					return;
				}
			}
			catch (...)
			...
			try
			{
				if (mConverterParams.mNumericLocale)
				{
					...
					{
						pVal = static_cast<T>(strtof(pStr.c_str(), NULL));
						return;
					}
					else if (typeid(T) == typeid(double))
					{
						pVal = static_cast<T>(strtod(pStr.c_str(), NULL));
						return;
					}
					else if (typeid(T) == typeid(long double))
					{
						pVal = static_cast<T>(strtold(pStr.c_str(), NULL));
						return;
					}
				}
				else
	...
	struct SeparatorParams
	{
		...
		explicit SeparatorParams(const char pSeparator = ',',
			...
			: mSeparator(pSeparator), mTrim(pTrim), mHasCR(pHasCR), mQuotedLinebreaks(pQuotedLinebreaks),
			  mAutoQuote(pAutoQuote), mQuoteChar(pQuoteChar)
		{
		}
	...
	class Document
	{
	  public:
		...
		explicit Document(const std::string& pPath = std::string(),
			...
			: mPath(pPath), mLabelParams(pLabelParams), mSeparatorParams(pSeparatorParams),
			  mConverterParams(pConverterParams), mLineReaderParams(pLineReaderParams), mData(), mColumnNames(),
			  mRowNames()
		{
			if (!mPath.empty())
			{
		...
		std::vector<T> GetColumn(const std::string& pColumnName) const
		{
			...
			if (columnIdx < 0)
			{
				warn("column not found: %s", pColumnName.c_str());
				std::vector<T> empty;
				return empty;
			}
		...
		{
			const int count =
				static_cast<int>((mData.size() > 0) ? mData.at(0).size() : 0) - (mLabelParams.mRowNameIdx + 1);
		...
		std::vector<T> GetRow(const std::string& pRowName) const
		{
			...
			if (rowIdx < 0)
			{
				warn("row not found: %s", pRowName.c_str());
		...
		void SetRow(const size_t pRowIdx, const std::vector<T>& pRow)
		{
			...
			for (auto itCol = pRow.begin(); itCol != pRow.end(); ++itCol)
			{
				...
				mData.at(dataRowIdx)
					.at(static_cast<size_t>(std::distance(pRow.begin(), itCol) + mLabelParams.mRowNameIdx + 1)) = str;
			}
		}
					   ...
					   const std::string& pRowName = std::string())
		{
			...
			else
			{
				...
				for (auto itCol = pRow.begin(); itCol != pRow.end(); ++itCol)
				{
					std::string str;
					converter.ToStr(*itCol, str);
					row.at(static_cast<size_t>(std::distance(pRow.begin(), itCol) + mLabelParams.mRowNameIdx + 1)) =
						str;
				}
			}
		...
		T GetCell(const size_t pColumnIdx, const size_t pRowIdx) const
		{
			const size_t dataColumnIdx = GetDataColumnIndex(pColumnIdx);
			const size_t dataRowIdx = GetDataRowIndex(pRowIdx);
			dbg("dataColumnIdx: %d, dataRowIdx: %d", dataColumnIdx, dataRowIdx);
		...
		std::string GetColumnName(const size_t pColumnIdx) const
		{
			...
			if (mLabelParams.mColumnNameIdx < 0)
			{
				warn("column name row index < 0: %d", mLabelParams.mColumnNameIdx);
		...
		std::vector<std::string> GetColumnNames() const
		{
			...
			if (mLabelParams.mColumnNameIdx >= 0)
			{
				if (mLabelParams.mColumnNameIdx >= static_cast<int>(mData.size()))
				{
					warn("column name row index >= data size: %d >= %d", mLabelParams.mColumnNameIdx, mData.size());
					return std::vector<std::string>();
				}
				return std::vector<std::string>(mData[static_cast<size_t>(mLabelParams.mColumnNameIdx)].begin() +
													(mLabelParams.mRowNameIdx + 1),
												mData[static_cast<size_t>(mLabelParams.mColumnNameIdx)].end());
			}
		...
		std::string GetRowName(const size_t pRowIdx) const
		{
			...
			if (mLabelParams.mRowNameIdx < 0)
			{
				warn("row name column index < 0: %d", mLabelParams.mRowNameIdx);
	  ...
	  private:
		void ReadCsv()
		{
			...
			stream.open(mPath.c_str());
			info("Reading CSV file: %s", mPath.c_str());
			ReadCsv(stream);
		}
		...
		void ReadCsv(std::istream& pStream)
		{
			...
			if ((bom2b == bomU16le) || (bom2b == bomU16be))
			{
				...
				{
					wstream.imbue(
						std::locale(wstream.getloc(), new std::codecvt_utf16<wchar_t, 0x10ffff, std::consume_header>));
				}
		...
		void WriteCsv(std::ostream& pStream) const
		{
			for (auto itr = mData.begin(); itr != mData.end(); ++itr)
			{
				...
				{
					if (mSeparatorParams.mAutoQuote && ((itc->find(mSeparatorParams.mSeparator) != std::string::npos) ||
		...
		{
			mColumnNames.clear();
			if ((mLabelParams.mColumnNameIdx >= 0) && (static_cast<int>(mData.size()) > mLabelParams.mColumnNameIdx))
			{
				size_t i = 0;
		...
		{
			mRowNames.clear();
			if ((mLabelParams.mRowNameIdx >= 0) && (static_cast<int>(mData.size()) > (mLabelParams.mColumnNameIdx + 1)))
			{
				size_t i = 0;
