...
* All the logic is contained in the `src/jni/` folder.
* Flythings IDE will automatically generate some code in the `activity` and `logic` folders.


| Folder         | Description                                                                                                                                  |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `activity`     | Generated by Flythings IDE. **DO NOT MODIFY**                                                                                                |
| `comm`         | Code involved in communication                                                                                                               |
| `Hardware`     | Code for interacting with specific hardware                                                                                                  |
| `include`      | Header files for Flythings and generic libraries                                                                                             |
| `logic`        | Generated by Flythings IDE. When you add a GUI element, the asscoiated functions to control it will be added here.                           |
| `ObjectModel`  | Code for maintaining a local copy of the Duet Object Model                                                                                   |
| `uart`         | Code for interacting with the UART                                                                                                           |
| `UI`           | Code for setting the GUI contents and responding to events                                                                                   |
| `UI/Guides`    | Location for any user guides / tutorials that can be displayed on the screen                                                                 |
| `UI/Logic`     | Code for the logic of the UI elements. This is to keep the `mainLogic.cc` file clean. Generally there is one file per main window of the GUI |
| `UI/Observers` | Code for observing the Duet Object Model and updating the GUI accordingly                                                                    |
| `UI/Themes`    | Location for any themes that can be applied to the screen                                                                                    |
| `upgrade`      | Code for upgrading the firmware                                                                                                              |
| `utils`        | Utility functions that are used across the codebase                                                                                          |
